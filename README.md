...
| `/vm_add_product` | 自動販売機に新しい商品を追加します | `vending_machine_id`, `name`, `price`, `product_url`, `stock_type`, `stock_count`, `delivery_type`, `description`（任意）, `emoji`（任意） |
| `/vm_add_stock` | 商品に在庫を追加します | `vending_machine_id`, `stock_file`: .txtファイル |
...
| `/vm_update` | 自動販売機パネルを更新します | `vending_machine_id`, `message_link`, `panel_title`, `panel_description`, `panel_image`, `color` |
...
| **無限** | 固定テキストを無限に販売可能 |
| **TXTランダム** | .txtファイルからランダムに1行排出 |

### 配布方法

| 方法 | 説明 |
|------|------|
| **DMに送る** | 購入完了後、商品をDMで送信 |
| **チケット作成** | 専用チケットチャンネルを作成して商品を送信 |
| **TXTからランダム排出** | .txtファイルからランダムに1行を抽出してDM送信 |
...
| コマンド | 説明 | パラメータ |
|---------|------|-----------|
| `/achi` | 実績パネルを設置します | `channel`: 送信先チャンネル, `name`: 実績名, `description`: 実績の説明, `image_url`（任意） |
...
| コマンド | 説明 | パラメータ |
|---------|------|-----------|
| `/image_verify` | 画像認証パネルを設置します | `role`: 認証成功時に付与するロール |
